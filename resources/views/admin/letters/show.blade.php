            ...
            <div class="bg-light rounded h-100 p-4">
                ...
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge {{ $letter->status == 'pending' ? 'bg-warning' : ($letter->status == 'approved' || $letter->status == 'printed' ? 'bg-success' : ($letter->status == 'completed' ? 'bg-info' : 'bg-danger')) }}">
                            {{ $letter->status == 'pending' ? 'Menunggu' : ($letter->status == 'approved' ? 'Disetujui' : ($letter->status == 'printed' ? 'Dicetak' : ($letter->status == 'completed' ? 'Selesai' : 'Ditolak'))) }}
                        </span>
                    </div>
                    <div class="card-body">
                        <div class="row mb-3">
