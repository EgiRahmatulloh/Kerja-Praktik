...
@section('title', 'Form Pengisian Surat')
    ...
    <div class="row g-4">
        <div class="col-12">
            <div class="bg-light rounded h-100 p-4">
                <h4 class="mb-4">Form Pengisian Surat {{ $letterType->name }}</h4>

                <form action="{{ route('admin.letters.store', $letterType->id) }}" method="POST">
                    @csrf
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group">
                                ...
                                <select class="form-select @error($item->key) is-invalid @enderror"
                                    ...
                                    <option value="{{ $option }}" {{ old($item->key) == $option ? 'selected' : '' }}>
                                        {{ $option }}
                                    </option>
                                    @endforeach
                                </select>
                                ...
                                <textarea class="form-control @error($item->key) is-invalid @enderror text-input"
                                    id="{{ $item->key }}" name="{{ $item->key }}"
                                    rows="3" {{ $item->required ? 'required' : '' }}>{{ old($item->key) }}</textarea>
                                @endif
                    ...
                    <div class="mt-4">
                        <a href="{{ route('admin.letters.index') }}" class="btn btn-secondary">Kembali</a>
