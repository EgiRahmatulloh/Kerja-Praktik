                ...
                <form action="{{ route('admin.templates.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="share_setting" name="share_setting">
                            <option value="private" {{ old('share_setting') == 'private' ? 'selected' : '' }}>Pribadi (Hanya saya)</option>
                            <option value="public" {{ old('share_setting') == 'public' ? 'selected' : '' }}>Publik (Semua admin)</option>
                            <option value="limited" {{ old('share_setting') == 'limited' ? 'selected' : '' }}>Terbatas (Pilih admin)</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3" id="shared_users_container">
                        ...
                        <select class="form-select" id="shared_users" name="shared_users[]" multiple>
                            @foreach ($admins as $admin)
                                <option value="{{ $admin->id }}" {{ in_array($admin->id, old('shared_users', [])) ? 'selected' : '' }}>
